...
/**
 * Creates a blob URL from a zip file (deprecated)
 */
export function createZipBlobUrl(zipBlob: Blob): string {
  console.warn("createZipBlobUrl is deprecated, use createPdfBlobUrl instead");
  return URL.createObjectURL(zipBlob);
}

/**
 * Triggers a download of a zip file (deprecated)
 */
export function downloadZip(zipUrl: string, filename: string = 'school-results.zip'): void {
  console.warn("downloadZip is deprecated, use downloadPdf instead");
  const a = document.createElement('a');
  a.href = zipUrl;
  a.download = filename;
  document.body.appendChild(a);
